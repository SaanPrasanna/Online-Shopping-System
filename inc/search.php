<?php 
	...
	$sql = "SELECT * FROM product WHERE name LIKE '%{$search}%' AND remove = 0;";
	...
	if(mysqli_num_rows($result) > 0){
		while($row = mysqli_fetch_assoc($result)){		
			...
			if ($row['u_price'] != $price) {
				$update = '<s class="text-secondary">Rs. ' . $row['u_price'] . ' </s> &nbsp
			...
			$output .='<div class="col-md-3 col-sm-6 my-3 my-md-0" >
						<form>
		...
		$output .= "<span class='display-4 text-danger mx-auto'><i class='fas fa-sync'></i> Sorry, ".ucfirst($search)." Not Found.</span>";
	}
	$output .= "</div></div>";
	echo $output;
